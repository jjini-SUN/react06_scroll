//Scroll-indicator 라는 컴포넌트를 만들어서
//App.js에 붙이기
...
export default function ScrollIndicator({url}) {
  ...
  async function fetchData(url) {
    try{
      //서버에 요청을 하기 정에 로딩 상태로 만든다
      setLoading(true);  //로딩 상태 ON

      let res = await fetch(url);  //get 요청
      //처리가 완료될 때까지 기다리기 위해 await (아래 코드로 넘어가지 않고 대기해줌)
  ...
  }, [url]); //url이 바뀔 때마다 실행 (안적으면 모든 데이터에 대해 실행)
  ...
  }, [])  //[]: update는 동작 안하게 (처음만 동작)

  function changeScrollEvent() {
    //스크롤의 위치를 감지 ==> 현재 스크롤 위치
    let scrolled = document.documentElement.scrollTop;
    //창이 작을 수도 있으니깐 현재 열려있는 창의 스크롤 범위를 계산 => 전체 스크롤 가능한 범위
  ...
  return(
    <>
      ...
      <div className='data-list'>
        {
          //data가 100개 짜리 배열이니깐
          //data에 map을 통해 p 태그로 e.title을 출력
          // <p>{e.title}</p>
          //data가 비어있지 않고 길이가 0보다 클 때 p태그 생성
          data && data.length > 0 ? 
          data.map((e, idx)=>{ //e: data[0]~data[99] / idx: 0~99
            return(
              <p key={idx}>{e.title}</p>
            )
          })
